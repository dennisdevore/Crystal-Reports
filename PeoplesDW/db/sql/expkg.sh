...
spool ${SYN_LOWER}syn.sql;

select '--' || CHR(10) ||
       '-- \$Id: ${SYN_LOWER}syn.sql 1483 2008-09-21 11:46:02Z brianb $' || CHR(10) ||
       '--'
  from dual;

select 'create or replace public synonym ${SYN_LOWER} for ' ||
       rtrim(lower(table_name)) || ';'
  from all_synonyms
 where table_owner = 'ALPS'
   and synonym_name = '${SYN_UPPER}';
