set serveroutput on;
spool change_lot_options.out;

create or replace PROCEDURE tmp_inventory_adjustment
(in_lpid varchar2
,in_custid varchar2
,in_item varchar2
            ...
            if strWhse is null then
              insert into invadjactivity
               ...
               (l_sysdate, in_lpid, pl.facility, new.custid, new.item, new.lotnumber,
                ...
                in_tasktype, in_userid, in_userid, sysdate,
                pl.custid, pl.item, pl.lotnumber,
                pl.inventoryclass, pl.invstatus, 0, in_custreference, in_suppress_edi_yn)
                  returning rowid into adjrowid;
              out_adjrowid1 := adjrowid;
            end if;
         end if;
...
end tmp_inventory_adjustment;
/
show errors procedure tmp_inventory_adjustment;

declare

cursor curSelectedLips(in_custid varchar2) is
  select lpid,
         custid,
         item,
         nvl(inventoryclass,'RG') as inventoryclass,
...
l_plate_tot_cnt pls_integer := 0;
l_plate_oky_cnt pls_integer := 0;
l_plate_err_cnt pls_integer := 0;
l_plate_tot_qty pls_integer := 0;
l_plate_oky_qty pls_integer := 0;
l_plate_err_qty pls_integer := 0;
l_orderhdr_tot_cnt pls_integer := 0;
l_orderdtl_tot_cnt pls_integer := 0;
l_orderdtl_oky_cnt pls_integer := 0;
l_orderdtl_err_cnt pls_integer := 0;
l_orderdtl_tot_qty pls_integer := 0;
l_orderdtl_oky_qty pls_integer := 0;
l_orderdtl_err_qty pls_integer := 0;
l_update char(1);
...
if l_update = 'Y' then
  update custitem
   ...
   where custid = l_custid
     and (nvl(lotrequired,'N') != 'C' or
          nvl(user1required,'N') != 'C');
  ...
  update customer
     set lotrequired = 'N',
         user1required = 'Y'
   where custid = l_custid
     and (nvl(lotrequired,'N') != 'N' or
          nvl(user1required,'N') != 'Y');
end if;

for pl in curSelectedLips(l_custid)
loop

  l_plate_tot_cnt := l_plate_tot_cnt + 1;
  l_plate_tot_qty := l_plate_tot_qty + pl.qty;
  
  zut.prt(pl.lpid || ' ' || pl.item || ' ' || pl.lotnumber || ' qty: ' || pl.qty);
  
  if (l_update = 'Y') then
	  tmp_inventory_adjustment
	  (pl.lpid
	  ,pl.custid
	  ,pl.item
	  ,pl.inventoryclass
    ...
    ,null);
	  ...
	  if out_errorno != 0 then
	    rollback;
	    l_plate_err_cnt := l_plate_err_cnt + 1;
	    l_plate_err_qty := l_plate_err_qty + pl.qty;
	    zut.prt(out_msg);
	  else
	    commit;
	    l_plate_oky_cnt := l_plate_oky_cnt + 1;
	    l_plate_oky_qty := l_plate_oky_qty + pl.qty;
	  end if;
...
for oh in (select orderid,shipid,orderstatus,fromfacility
             from orderhdr
            where ordertype = 'O'
              and orderstatus < '6'
              and custid = '1043')
loop
  ...
  zut.prt(oh.orderid || '-' || oh.shipid);
  
  l_orderhdr_tot_cnt := l_orderhdr_tot_cnt + 1;
  
  for od in (select item,lotnumber,qtyorder
               from orderdtl
  ...
  loop
    ...
    l_orderdtl_tot_cnt := l_orderdtl_tot_cnt + 1;
    zut.prt(od.item || ' ' || od.lotnumber);
    ...
    if l_update = 'Y' then
      ...
      update orderdtl
       ...
       where orderid = oh.orderid
         ...
         and item = od.item
         and lotnumber = od.lotnumber;
      l_orderdtl_oky_cnt := l_orderdtl_oky_cnt + 1;
      l_orderdtl_oky_qty := l_orderdtl_oky_qty + od.qtyorder;

      commit;
      
      if oh.orderstatus >= '3' then
        zoe.regenerate_picks(oh.orderid, oh.shipid, od.item, od.lotnumber,
              oh.fromfacility, 'SYSTEM', out_msg);
        if substr(out_msg, 1, 4) != 'OKAY' then
          rollback;
          zut.prt(out_msg);
        else
...
zut.prt('plate tot ' || l_plate_tot_cnt || ' qty: ' || l_plate_tot_qty);
zut.prt('plate oky ' || l_plate_oky_cnt || ' qty: ' || l_plate_oky_qty);
zut.prt('plate err ' || l_plate_err_cnt || ' qty: ' || l_plate_err_qty);
zut.prt('orderhdr tot ' || l_orderhdr_tot_cnt);
zut.prt('orderdtl tot ' || l_orderdtl_tot_cnt || ' qty: ' || l_orderdtl_tot_qty);
zut.prt('orderdtl oky ' || l_orderdtl_oky_cnt || ' qty: ' || l_orderdtl_oky_qty);
zut.prt('orderdtl err ' || l_orderdtl_err_cnt || ' qty: ' || l_orderdtl_err_qty);
...
/
drop procedure tmp_inventory_adjustment;
exit;