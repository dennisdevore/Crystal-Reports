...
from
  ...
  where orderdtlrcpt.orderid = orderhdr.orderid
    and orderdtlrcpt.shipid = orderhdr.shipid))
...
zut.prt('begin orphan orderdtlrcpt row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'orderdtlrcpt purged: ' || cntorderdtlrcpt, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('orderdtlrcpt count is ' || cntorderdtlrcpt);
    ...
    if cntorderdtlrcpt >= maxRowcount then
...
cntTotRowsPurged := cntorderdtlrcpt;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan orderdtlrcpt row purge...');
rollback;
