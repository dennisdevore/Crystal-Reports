...
from
  (select distinct orderid,shipid
    from multishipdtl
   where to_date(shipdatetime,'YYYYMMDDHH24MISS') < in_cutoff
     and not exists
      (select 1 from multishiphdr
        where multishipdtl.orderid = multishiphdr.orderid
          and multishipdtl.shipid = multishiphdr.shipid)
    and status in ('PROCESSED','VOID'))
...
zut.prt('begin orphan multishipdtl row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'multishipdtl purged: ' || cntmultishipdtl, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('multishipdtl count is ' || cntmultishipdtl);
    ...
    if cntmultishipdtl >= maxRowcount then
...
cntTotRowsPurged := cntmultishipdtl;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan multishipdtl row purge...');
rollback;
