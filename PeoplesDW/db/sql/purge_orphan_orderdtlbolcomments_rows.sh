...
from
  ...
  where orderdtlbolcomments.orderid = orderhdr.orderid
    and orderdtlbolcomments.shipid = orderhdr.shipid))
...
zut.prt('begin orphan orderdtlbolcomments row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'orderdtlbolcomments purged: ' || cntorderdtlbolcomments, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('orderdtlbolcomments count is ' || cntorderdtlbolcomments);
    ...
    if cntorderdtlbolcomments >= maxRowcount then
...
cntTotRowsPurged := cntorderdtlbolcomments;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan orderdtlbolcomments row purge...');
rollback;
