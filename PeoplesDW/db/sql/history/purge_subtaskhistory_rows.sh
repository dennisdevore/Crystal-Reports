...
zut.prt('begin SubTaskHistory row purge...');
...
zut.prt('Update flag is ' || update_flag);
  ...
  loop
    ...
    if (cntRows) > 0 then
      if update_flag = 'Y' then
        ...
        zms.log_autonomous_msg('PURGE', null, null,
            'SubTaskHistory purged: ' || cntTaskHistory, 'I', 'PURGE', strOutMsg);
      else
    ...
    zut.prt('SubTaskHistory count is ' || cntTaskHistory);
...
zut.prt('SubTaskHistory count ' || cntTaskHistory);
cntTotRowsPurged := cntTaskHistory;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end SubTaskHistory row purge...');
rollback;
