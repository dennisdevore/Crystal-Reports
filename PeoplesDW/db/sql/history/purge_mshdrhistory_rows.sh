...
type mshdrhistory_tbl_type is table of rowid;

mshdrhistory_tbl mshdrhistory_tbl_type;
...
zut.prt('begin MultiShipHdrHistory row purge...');
...
zut.prt('Update flag is ' || update_flag);
  ...
  loop
    ...
    fetch curPurgableRecords bulk collect into mshdrhistory_tbl limit maxFetchLimit;

    if mshdrhistory_tbl.count = 0 then
      exit;
    end if;

    forall i in mshdrhistory_tbl.first .. mshdrhistory_tbl.last
      delete multishiphdrhistory
       where rowid = mshdrhistory_tbl(i);
    ...
    if (cntRows) > 0 then
      if update_flag = 'Y' then
        ...
        zms.log_autonomous_msg('PURGE', null, null,
            'MultiShipHdrHistory purged: ' || cntTableHistory, 'I', 'PURGE', strOutMsg);
      else
    ...
    zut.prt('MultiShipHdrHistory count is ' || cntTableHistory);
...
zut.prt('MultiShipHdrHistory count ' || cntTableHistory);
cntTotRowsPurged := cntTableHistory;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end MultiShipHdrHistory row purge...');
rollback;
