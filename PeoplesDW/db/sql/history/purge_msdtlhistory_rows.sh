...
type msdtlhistory_tbl_type is table of rowid;

msdtlhistory_tbl msdtlhistory_tbl_type;
...
zut.prt('begin MultiShipDtlHistory row purge...');
...
zut.prt('Update flag is ' || update_flag);
  ...
  loop
    ...
    fetch curPurgableRecords bulk collect into msdtlhistory_tbl limit maxFetchLimit;

    if msdtlhistory_tbl.count = 0 then
      exit;
    end if;

    forall i in msdtlhistory_tbl.first .. msdtlhistory_tbl.last
      delete multishipdtlhistory
       where rowid = msdtlhistory_tbl(i);
    ...
    if (cntRows) > 0 then
      if update_flag = 'Y' then
        ...
        zms.log_autonomous_msg('PURGE', null, null,
            'MultiShipDtlHistory purged: ' || cntTableHistory, 'I', 'PURGE', strOutMsg);
      else
    ...
    zut.prt('MultiShipDtlHistory count is ' || cntTableHistory);
...
zut.prt('MultiShipDtlHistory count ' || cntTableHistory);
cntTotRowsPurged := cntTableHistory;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end MultiShipDtlHistory row purge...');
rollback;
