...
zut.prt('begin TaskHistory row purge...');
...
zut.prt('Update flag is ' || update_flag);
  ...
  loop
    ...
    if (cntRows) > 0 then
      if update_flag = 'Y' then
        ...
        zms.log_autonomous_msg('PURGE', null, null,
            'TaskHistory purged: ' || cntTaskHistory, 'I', 'PURGE', strOutMsg);
      else
    ...
    zut.prt('TaskHistory count is ' || cntTaskHistory);
...
zut.prt('TaskHistory count ' || cntTaskHistory);
cntTotRowsPurged := cntTaskHistory;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end TaskHistory row purge...');
rollback;
