...
from
  (select orderid,shipid
   ...
   where lastupdate < in_cutoff
     and ( shipid != 0 and not exists
           (select 1
              from orderhdr
             where shippingplate.orderid = orderhdr.orderid
               and shippingplate.shipid = orderhdr.shipid)
         )
      or ( shipid = 0 and not exists
           (select 1
              from waves
             where shippingplate.orderid = waves.wave)
         )
     and status in ('SH')
  )
...
zut.prt('begin orphan shippingplate row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'shippingplate purged: ' || cntshippingplate, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('shippingplate count is ' || cntshippingplate);
    ...
    if cntshippingplate >= maxRowcount then
...
cntTotRowsPurged := cntshippingplate;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan shippingplate row purge...');
rollback;
