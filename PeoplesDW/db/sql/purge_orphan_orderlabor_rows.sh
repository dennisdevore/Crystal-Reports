...
zut.prt('begin orphan orderlabor row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'orderlabor purged: ' || cntorderlabor, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('orderlabor count is ' || cntorderlabor);
    ...
    if cntorderlabor >= maxRowcount then
...
cntTotRowsPurged := cntorderlabor;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan orderlabor row purge...');
rollback;
