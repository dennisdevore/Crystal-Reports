...
type plate_table_type is table of shippingaudit.lpid%type;
...
from
   (select lpid
     from shippingaudit
    where auditdate < in_cutoff
      and not exists
    (select * from shippingplate
    where shippingaudit.lpid = shippingplate.fromlpid)
      and not exists
    (select * from shippingplate
    where shippingaudit.lpid = shippingplate.lpid)
   )
...
zut.prt('begin orphan shippingaudit row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'shippingaudit purged: ' || cntshippingaudit, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('shippingaudit count is ' || cntshippingaudit);
    ...
    if cntshippingaudit >= maxRowcount then
...
cntTotRowsPurged := cntshippingaudit;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan shippingaudit row purge...');
rollback;
