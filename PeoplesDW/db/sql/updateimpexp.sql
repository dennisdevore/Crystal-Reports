...
for x in curImpExp loop
  ...
  if substr(upper(x.deffilename),1,11) = 'G:\OHLPROD\' then
    zut.prt('updating . . ');
    update impexp_definitions
       set deffilename = 'N:\OHLTRAIN\' || substr(x.deffilename,12,255)
     where rowid = x.rowid;
  ...
  if substr(upper(x.deffilename),1,25) = 'G:\HP\EDI\INBOUND\ORDERS\' then
    zut.prt('updating . . ');
    update impexp_definitions
       set deffilename = 'N:\OHLTRAIN\IMPORT\INCOMING\' || substr(x.deffilename,26,255)
     where rowid = x.rowid;
