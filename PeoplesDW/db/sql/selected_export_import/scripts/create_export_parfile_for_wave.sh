...
into l_has_loads
from orderhdr
where loadno is not null and wave = l_wave;
        ...
        if l_table_count != 0 then
          case l_txt_file_name
            when 'parfiles_for_order/by_custid_tables.txt' then
              l_query_clause := 'where custid = ''DEFAULT'' or custid in (select distinct custid from orderhdr where wave = ' || l_wave || ')';
            when 'parfiles_for_order/by_custworkorder_tables.txt' then
              l_query_clause := 'where seq in (select distinct seq from custworkorder ' ||
                                               'where custid in (select distinct custid from orderhdr where wave = ' || l_wave || '))';
            when 'parfiles_for_order/by_facility_tables.txt' then
              l_query_clause := 'where facility = ''' || l_wave_row.facility || '''';
            when 'parfiles_for_order/by_facility_custid_tables.txt' then
              l_query_clause := 'where facility = ''' || l_wave_row.facility || '''' ||
                                ' and custid in (select distinct custid from orderhdr where wave = ' || l_wave || ')';
            when 'parfiles_for_order/by_facility_custid_orderid_tables.txt' then
              l_query_clause := 'where facility = ''' || l_wave_row.facility || '''' ||
                                ' and custid in (select distinct custid from orderhdr where wave = ' || l_wave || ')' ||
                                ' and (orderid, shipid) in (select orderid, shipid from orderhdr where wave = ' || l_wave || ')';
            when 'parfiles_for_order/by_invoice_tables.txt' then
              l_query_clause := 'where invoice in (select distinct invoice from ' ||
                                'invoicedtl where (orderid, shipid) in (select orderid, shipid from orderhdr where wave = ' || l_wave || '))';
            when 'parfiles_for_order/by_loadno_tables.txt' then
              if l_has_loads > 0 then
                l_query_clause := 'where loadno in (select distinct loadno from orderhdr
                                                    where wave = ' || l_wave || ' and loadno is not null)';
              else
                l_query_clause := 'where 1 = 0';
              end if;
            when 'parfiles_for_order/by_orderid_tables.txt' then
              l_query_clause := 'where (orderid, shipid) in (select orderid, shipid from orderhdr where wave = ' || l_wave || ')'; 
            when 'parfiles_for_order/by_orderid_noshipid.txt' then
              l_query_clause := 'where (orderid) in (select orderid from orderhdr where wave = ' || l_wave || ')';
            when 'parfiles_for_order/by_plate_tables.txt' then
              l_query_clause := 'where (custid in (select distinct custid from orderhdr where wave = ' || l_wave || ')' ||
                                ' and type != ''XP'') ' ||
                                'or (custid in (select distinct custid from orderhdr where wave = ' || l_wave || ')' ||
                                ' and type = ''XP'' and lpid in (select ' ||
                                'distinct fromlpid from shippingplate ' ||
                                'where (orderid, shipid) in (select orderid, shipid from orderhdr where wave = ' || l_wave || ')' || 
