...
ls -1 parfiles_for_order/*.txt > dumps/txtfiles_for_order$$.out
...
begin
  l_fty_filelist := utl_file.fopen('SYNAPSE_$$_DUMPS','txtfiles_for_order$$.out','r',32767);
exception when others then
  dbms_output.put_line(sqlerrm);
  dbms_output.put_line('txtfiles_for_order$$.out open error');
  return;
end;

--dbms_output.put_line('begin txtfiles_for_order processing...');
while(1=1)
loop
  begin
...
sqls @/tmp/$IAM.$$.sql
rm /tmp/$IAM.$$.sql
rm dumps/txtfiles_for_order$$.out
