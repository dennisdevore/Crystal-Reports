   ...
   for fa in C_FAC loop   
      for cit in C_CUSTITEM loop   
         for crec in C_ITEMS(fa.facility, cit.custid, cit.item, cit.lotrequired, startdate) loop
            ...
            if (lastasof.currentqty != lp.quantity + sp.quantity) or
               (lastasof.currentweight != lp.weight + sp.weight) then
               asoftrans := null;
               ...
               if(asoftrans.adjustment != lastasof.currentqty) or
                 (asoftrans.weightadjustment != lastasof.currentweight)then
                  for casofdaily in C_ASOFDAILY(fa.facility, cit.custid, cit.item,
                     ...
                     if (casofp.currentqty is not null) and
                     	  ((casofp.currentqty != casofdaily.previousqty) or (casofp.currentweight != casofdaily.previousweight)) then
                     	  if (casofp.currentqty != casofdaily.previousqty) then
                           ...
                           zut.prt('ASOF: '||fa.facility||'/'||cit.custid
                                 ...
                                 ||'/'||crec.invstatus
                                 ||'/'||crec.inventoryclass
                                 ||' CW= '||to_char(casofp.currentweight)
                                 ||' PW= '||to_char(casofdaily.previousweight));
                        end if;
                     ...
                     if ((casofdaily.currentqty - casofdaily.previousqty) != casofdailytrans.adjustment) or
                        ((casofdaily.currentweight - casofdaily.previousweight) != casofdailytrans.weightadjustment) then
                        if (casofdaily.currentqty - casofdaily.previousqty) != casofdailytrans.adjustment then
                           ...
                           zut.prt('ASOF: '||fa.facility||'/'||cit.custid
                                 ...
                                 ||'/'||crec.inventoryclass
                                 ||' PW= '||to_char(casofdaily.previousweight)
                                 ||' + '||to_char(casofdailytrans.weightadjustment)
                                 ||' CW='||to_char(casofdaily.currentweight));
                        end if;
            ...
            if (lastasof.currentqty != lp.quantity + sp.quantity) or
               ...
               if (lastasof.currentqty != lp.quantity + sp.quantity) then
                  ...
                  zut.prt('FOR: '||fa.facility||'/'||cit.custid
                        ...
                        ||' = '||to_char(lp.weight)
                        ||' + '||to_char(sp.weight)
                        ||' CW='||to_char(lastasof.currentweight));
               end if;
