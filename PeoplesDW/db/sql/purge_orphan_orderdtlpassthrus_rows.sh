...
from
  ...
  where orderdtlpassthrus.orderid = orderhdr.orderid
    and orderdtlpassthrus.shipid = orderhdr.shipid))
...
zut.prt('begin orphan orderdtlpassthrus row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'orderdtlpassthrus purged: ' || cntorderdtlpassthrus, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('orderdtlpassthrus count is ' || cntorderdtlpassthrus);
    ...
    if cntorderdtlpassthrus >= maxRowcount then
...
cntTotRowsPurged := cntorderdtlpassthrus;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan orderdtlpassthrus row purge...');
rollback;
