...
from
  (select distinct orderid,shipid
    from orderhistory
   where chgdate < in_cutoff
     and not exists
  (select 1 from orderhdr
  where orderhistory.orderid = orderhdr.orderid
    and orderhistory.shipid = orderhdr.shipid))
...
zut.prt('begin orphan orderhistory row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'orderhistory purged: ' || cntorderhistory, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('orderhistory count is ' || cntorderhistory);
    ...
    if cntorderhistory >= maxRowcount then
...
cntTotRowsPurged := cntorderhistory;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan orderhistory row purge...');
rollback;
