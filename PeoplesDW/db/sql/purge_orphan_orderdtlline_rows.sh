...
from
  ...
  where orderdtlline.orderid = orderhdr.orderid
    and orderdtlline.shipid = orderhdr.shipid))
...
zut.prt('begin orphan orderdtlline row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'orderdtlline purged: ' || cntorderdtlline, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('orderdtlline count is ' || cntorderdtlline);
    ...
    if cntorderdtlline >= maxRowcount then
...
cntTotRowsPurged := cntorderdtlline;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan orderdtlline row purge...');
rollback;
