...
from
  ...
  where orderhdrpassthrus.orderid = orderhdr.orderid
    and orderhdrpassthrus.shipid = orderhdr.shipid))
...
zut.prt('begin orphan orderhdrpassthrus row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'orderhdrpassthrus purged: ' || cntorderhdrpassthrus, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('orderhdrpassthrus count is ' || cntorderhdrpassthrus);
    ...
    if cntorderhdrpassthrus >= maxRowcount then
...
cntTotRowsPurged := cntorderhdrpassthrus;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan orderhdrpassthrus row purge...');
rollback;
