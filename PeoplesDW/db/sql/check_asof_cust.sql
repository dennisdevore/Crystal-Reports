...
rc integer;
  ...
  CURSOR C_ITEMS(in_custid varchar2) IS
   ...
   union
      select distinct facility, custid, item, lotnumber, unitofmeasure,
        invstatus, inventoryclass
        from deletedplate where type = 'PA' and custid = in_custid
   union
     select distinct  facility, custid, item, lotnumber, unitofmeasure,
        invstatus, inventoryclass
...
begin
   ...
   for crec in C_ITEMS('23300') loop
       qty := 0;
