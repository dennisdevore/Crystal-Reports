  ...
  (select orderid,shipid
   ...
   where lastupdate < in_cutoff
     and not exists
           (select 1
              from orderhdr
             where asncartondtl.orderid = orderhdr.orderid
               and asncartondtl.shipid = orderhdr.shipid)
  )
...
zut.prt('begin orphan asncartondtl row purge...');
  ...
  loop
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'asncartondtl purged: ' || cntasncartondtl, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('asncartondtl count is ' || cntasncartondtl);
    ...
    if cntasncartondtl >= maxRowcount then
...
cntTotRowsPurged := cntasncartondtl;
zut.prt('total rows purged ' || cntTotRowsPurged);

zut.prt('end orphan asncartondtl row purge...');
rollback;
