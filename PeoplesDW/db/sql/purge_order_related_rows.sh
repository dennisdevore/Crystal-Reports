...
curRetentionYears integer := 5;
dteCutOff date;
dteBeginTime date;
cntRows integer := 0;
cntOrderHdr integer := 0;
cntOrderHdrBolComments integer := 0;
cntOrderAttach integer := 0;
cntOrderHistory integer := 0;
cntOrderHdrPassThrus integer := 0;
cntOrderAddr integer := 0;
...
zut.prt('begin order-related row purge...');
  ...
  loop
    ...
    forall i in orderid_tbl.first .. orderid_tbl.last
      delete orderattach
       where orderid = orderid_tbl(i)
         and shipid = shipid_tbl(i);
    cntRows := sql%rowcount;
    cntOrderattach := cntOrderattach + cntRows;
    ...
    forall i in orderid_tbl.first .. orderid_tbl.last
      delete orderaddr
       where orderid = orderid_tbl(i)
         and shipid = shipid_tbl(i);
    cntRows := sql%rowcount;
    cntorderaddr := cntorderaddr + cntRows;
    ...
    forall i in orderid_tbl.first .. orderid_tbl.last
      delete orderhdr
       where orderid = orderid_tbl(i)
         and shipid = shipid_tbl(i);
    cntRows := sql%rowcount;
    cntOrderHdr := cntOrderHdr + cntRows;
    ...
    if update_flag = 'Y' then
      ...
      zms.log_autonomous_msg('PURGE', null, null,
          'Order Hdrs purged: ' || cntOrderHdr, 'I', 'PURGE', strOutMsg);
    else
    ...
    zut.prt('Order count is ' || cntOrderHdr);
    ...
    if cntOrderHdr >= maxRowcount then
...
zut.prt('orderhdr count ' || cntOrderHdr);
zut.prt('orderhdrbolcomments count ' || cntOrderHdrBolComments);
zut.prt('orderattach count ' || cntorderattach);
zut.prt('orderhistory count ' || cntOrderHistory);
zut.prt('orderhdrpassthrus count ' || cntOrderHdrPassThrus);
zut.prt('orderaddr count ' || cntOrderAddr);
cntTotRowsPurged := cntOrderHdr
                  + cntOrderHdrBolComments
                  + cntorderattach
                  + cntOrderHistory
...
zut.prt('end order-related row purge...');
rollback;
