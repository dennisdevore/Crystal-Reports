create or replace PACKAGE BODY alps.zinvadj
IS
--
-- $Id$
--
...
begin
  ...
  inventory_adjustment(in_lpid, in_custid, in_item, in_inventoryclass, in_invstatus, in_lotnumber, in_serialnumber, in_useritem1, in_useritem2, in_useritem3,
    in_location, in_expirationdate, in_qty, in_orig_custid, in_orig_item, in_orig_inventoryclass, in_orig_invstatus, in_orig_lotnumber, in_orig_serialnumber,
    in_orig_useritem1, in_orig_useritem2, in_orig_useritem3, in_orig_location, in_orig_expirationdate, in_orig_qty, in_facility, in_adjreason, in_userid,
    in_tasktype, in_weight, in_orig_weight, in_mfgdate, in_orig_mfgdate, in_anvdate, in_orig_anvdate, pl.length, pl.length, 
    pl.width, pl.width, pl.height, pl.height, pl.pallet_weight, pl.pallet_weight, 
    out_adjrowid1, out_adjrowid2, out_errorno, out_msg, in_custreference, in_tasks_ok,
    in_suppress_edi_yn, in_sysdate, in_comments, in_adjust_picked_invstatus, in_adjust_orderdtlrcpt, in_calc_weight_from_item, in_calling_proc);
end inventory_adjustment;
...
nulldate date;
l_sysdate date;
l_newcustid plate.custid%type := null;
l_newitem plate.item%type := null;
l_newlotnumber plate.lotnumber%type := null;
l_newinventoryclass plate.inventoryclass%type := null;
l_newinvstatus plate.invstatus%type := null;
...
if (pl.status not in ('A','I','P','M') and zlbl.is_lp_unprocessed_autogen(in_lpid) = 'N') then
  out_msg := 'Invalid plate status: ' || pl.status;
  out_errorno := 19;
  return;
end if;

if pl.facility != in_facility then
  out_msg := 'Plate not at your facility: ' || pl.facility;
  out_errorno := 2;
  return;
end if;

strUserSetting := substr(zus.user_form_setting(in_userid,'PLATEADJUST',in_facility),1,12);
...
strUserSetting := substr(zus.user_form_setting(in_userid,'CriticalHolds',in_facility),1,12);
if strUserSetting <> 'SUPERVISOR' then
    if nvl(pl.invstatus,'xx') != nvl(in_invstatus,'xx') then
        l_cnt := 0;
        select count(1)
          into l_cnt
          from criticalholds
         where code in (nvl(pl.invstatus,'xx'), nvl(in_invstatus,'xx'));
        if nvl(l_cnt,0) > 0 then
            out_errorno := 82;
            out_msg := 'Not authorized to change Critical hold status.';
        end if;
    end if;
end if;
...
if in_length is null then
  new.length := pl.length;
else
  new.length := in_length;
end if;
...
if in_height is null then
  new.height := pl.height;
else
  new.height := in_height;
end if;
if in_pallet_weight is null then
  new.pallet_weight := pl.pallet_weight;
else
  new.pallet_weight := in_pallet_weight;
end if;
...
if pl.custid = new.custid and
   ...
   (ci.lotrequired in ('Y','O','S','A') or pl.lotnumber is null) and
   ...
   nvl(pl.length,0) = nvl(new.length,0) and
   nvl(pl.width,0) = nvl(new.width,0) and
   nvl(pl.height,0) = nvl(new.height,0) and
   nvl(pl.pallet_weight,0) = nvl(new.pallet_weight,0) and 
   pl.qty = new.qty and
...
if pl.status = 'P' then
   ...
   if nvl(in_adjust_picked_invstatus,'N') <> 'Y' then
   ...
   if pl.location != new.location then
...
if nvl(pl.length,0) != nvl(in_orig_length,0) then
  out_msg := 'The length on this plate record has changed--please review and retry';
  out_errorno := 4;
  return;
end if;

if nvl(pl.width,0) != nvl(in_orig_width,0) then
  out_msg := 'The width on this plate record has changed--please review and retry';
  out_errorno := 4;
  return;
end if;

if nvl(pl.height,0) != nvl(in_orig_height,0) then
  out_msg := 'The height on this plate record has changed--please review and retry';
  out_errorno := 4;
  return;
end if;

if nvl(pl.pallet_weight,0) != nvl(in_orig_pallet_weight,0) then
  out_msg := 'The pallet weight on this plate record has changed--please review and retry';
  out_errorno := 4;
  return;
end if;
...
if pl.custid != new.custid or
   ...
   rtrim(in_lotnumber) is null then
  select count(1)
    into cntRows
    from customer
   where custid = new.custid;
  ...
  if ci.lotrequired not in ('Y','O','S','A') then
    if rtrim(in_lotnumber) is null then
      new.lotnumber := null;
    else
...
if pl.location != new.location then
  ...
  exception when no_data_found then
    out_msg := 'Location not found: ' || pl.facility || '\' || new.location;
    out_errorno := 31;
    return;
  end;
...
if nvl(pl.length,0) != nvl(new.length,0) then
  if nvl(new.length,0) < 0 then
    out_msg := 'Adjusted length must be >= zero';
    out_errorno := 121;
    return;
  end if;
end if;

if nvl(pl.width,0) != nvl(new.width,0) then
  if nvl(new.width,0) < 0 then
    out_msg := 'Adjusted width must be >= zero';
    out_errorno := 121;
    return;
  end if;
end if;

if nvl(pl.height,0) != nvl(new.height,0) then
  if nvl(new.height,0) < 0 then
    out_msg := 'Adjusted height must be >= zero';
    out_errorno := 121;
    return;
  end if;
end if;

if nvl(pl.pallet_weight,0) != nvl(new.pallet_weight,0) then
  if nvl(new.pallet_weight,0) < 0 then
    out_msg := 'Adjusted pallet_weight must be >= zero';
    out_errorno := 121;
    return;
  end if;
end if;
...
if ci.use_catch_weights = 'Y' then
  if in_calc_weight_from_item = 'N' then
    if (new.weight <> pl.weight) and
	   (in_calling_proc is not null) and
       (in_calling_proc = 'MASS') then
	  out_msg := 'Item on plate is catch weight';
	  out_errorno := 39;
	  return;
	end if;
  else
...
--Do not adjust weight as per specs.  
--else
...
if (oh.orderstatus = 'A') or
   ...
   if (CIV_old.serialrequired not in ('Y','A') and CIV_old.serialasncapture = 'Y')
   or (CIV_old.user1required not in ('Y','A') and CIV_old.user1asncapture = 'Y')
   or (CIV_old.user2required not in ('Y','A') and CIV_old.user2asncapture = 'Y')
   or (CIV_old.user3required not in ('Y','A') and CIV_old.user3asncapture = 'Y')
   then
      CIV_old.asnflag := 'Y';
      if (pl.custid = new.custid) and
   ...
   or pl.item != new.item then
     ...
     if (CIV_new.serialrequired not in ('Y','A') and CIV_new.serialasncapture = 'Y')
     or (CIV_new.user1required not in ('Y','A') and CIV_new.user1asncapture = 'Y')
     or (CIV_new.user2required not in ('Y','A') and CIV_new.user2asncapture = 'Y')
     or (CIV_new.user3required not in ('Y','A') and CIV_new.user3asncapture = 'Y')
     then
        CIV_new.asnflag := 'Y';
     end if;
  ...
  if CIV_new.serialrequired in ('Y','A') then
  ...
  if CIV_new.user1required in ('Y','A') then
  ...
  if CIV_new.user2required in ('Y','A') then
  ...
  if CIV_new.user3required in ('Y','A') then
...
l_sysdate := nvl(in_sysdate, sysdate);
-- generate an adjustment activity row, unless the LiP is associated
-- with a an 'A'rrived order or an open return
if not ( (oh.orderstatus = 'A') or
  ...
  if pl.custid = new.custid and
     ...
     pl.inventoryclass = new.inventoryclass and
	 nvl(pl.manufacturedate,nulldate) = nvl(new.manufacturedate,nulldate) and
	 nvl(pl.expirationdate,nulldate) = nvl(new.expirationdate,nulldate) then
    if (pl.qty != new.qty) or (pl.weight != new.weight) then
      if in_tasktype = 'DM' then
         l_newcustid := in_custid;
         l_newitem := in_item;
         l_newlotnumber := in_lotnumber;
         l_newinventoryclass := in_inventoryclass;
         l_newinvstatus := in_invstatus;
      end if;
      insert into invadjactivity
       (whenoccurred, lpid, facility, custid, item, lotnumber,
        ...
        newcustid, newitem, newlotnumber, newinventoryclass, newinvstatus,
        suppress_edi_yn, comments, manufacturedate, expirationdate)
        values
       (l_sysdate, in_lpid, pl.facility, pl.custid, pl.item, pl.lotnumber,
        ...
        l_newcustid, l_newitem, l_newlotnumber, l_newinventoryclass, l_newinvstatus,
        in_suppress_edi_yn, in_comments, pl.manufacturedate, pl.expirationdate)
         ...
         if forceLotAdj = 'Y' then
            insert into invadjactivity
             (whenoccurred, lpid, facility, custid, item, lotnumber,
              ...
              suppress_edi_yn, comments, manufacturedate, expirationdate,
              newmanufacturedate, newexpirationdate)
              values
             (l_sysdate, in_lpid, pl.facility, pl.custid, pl.item, pl.lotnumber,
            pl.inventoryclass, pl.invstatus, pl.unitofmeasure, - new.qty, in_adjreason,
              in_tasktype, in_userid, in_userid, sysdate,
              new.custid, new.item, new.lotnumber,
            new.inventoryclass, new.invstatus, - new.weight, in_custreference,
              in_suppress_edi_yn, in_comments, pl.manufacturedate, pl.expirationdate,
              new.manufacturedate, new.expirationdate)
                returning rowid into adjrowid;
            out_adjrowid1 := adjrowid;
            insert into invadjactivity
             (whenoccurred, lpid, facility, custid, item, lotnumber,
              ...
              suppress_edi_yn, comments, manufacturedate, expirationdate,
              oldmanufacturedate, oldexpirationdate)
              values
             (l_sysdate, in_lpid, pl.facility, new.custid, new.item, new.lotnumber,
              ...
              in_suppress_edi_yn, in_comments, new.manufacturedate, new.expirationdate,
              pl.manufacturedate, pl.expirationdate)
                returning   rowid into adjrowid;
            ...
            if strWhse is null then
              insert into invadjactivity
               ...
               (l_sysdate, in_lpid, pl.facility, new.custid, new.item, new.lotnumber,
                ...
                in_tasktype, in_userid, in_userid, sysdate,
                pl.custid, pl.item, pl.lotnumber,
                pl.inventoryclass, pl.invstatus, 0, in_custreference, in_suppress_edi_yn, in_comments,
                new.manufacturedate, new.expirationdate, pl.manufacturedate, pl.expirationdate)
                  returning rowid into adjrowid;
              out_adjrowid1 := adjrowid;
         end if;
      end if;
    ...
    if in_tasktype != 'PI' then
      ...
      if pl.custid != new.custid or
        ...
        zmi3.get_whse(pl.custid,pl.inventoryclass,strWhse,strRegWhse,strRetWhse);
        if strWhse is not null then
          if nvl(pl.manufacturedate,nulldate) = nvl(new.manufacturedate,nulldate) or
             nvl(pl.expirationdate,nulldate) = nvl(new.expirationdate,nulldate) then
            out_msg := 'Manufacture or Expiration date changes must be performed independently of other changes';
            out_errorno := 37;
            goto undo_adjustment;
          end if;
        end if;
    ...
    insert into invadjactivity
     (whenoccurred, lpid, facility, custid, item, lotnumber,
      ...
      newcustid, newitem, newlotnumber,
      newinventoryclass, newinvstatus, adjweight, custreference,
      suppress_edi_yn, comments, newmanufacturedate, newexpirationdate,
    ...
    insert into invadjactivity
     (whenoccurred, lpid, facility, custid, item, lotnumber,
      ...
      oldcustid, olditem, oldlotnumber,
      oldinventoryclass, oldinvstatus, adjweight, custreference,
      suppress_edi_yn, comments, oldmanufacturedate, oldexpirationdate,
      manufacturedate, expirationdate)
      values
     (l_sysdate, in_lpid, pl.facility, new.custid, new.item, new.lotnumber,
      new.inventoryclass, new.invstatus, new.unitofmeasure, new.qty, in_adjreason,
...
update plate
   set custid = new.custid,
       ...
       width = new.width,
       height = new.height,
       pallet_weight = new.pallet_weight,
       quantity = new.qty,
       weight = new.weight,
...
-- orderdtlrcpt table
if (oh.orderstatus = 'A') or (in_adjust_orderdtlrcpt = 'Y') or
   (oh.ordertype = 'Q' and oh.orderstatus != 'X' and oh.orderstatus != 'R') then
  ...
  if (pl.custid = new.custid) and (pl.item = new.item) then
    if ci.use_catch_weights = 'Y' then
      ...
      if substr(out_msg, 1, 4) != 'OKAY' then
        zms.log_msg('PlateAdj', in_facility, new.custid, out_msg, 'W', in_userid, out_msg);
      end if;
      zcwt.add_item_lot_catch_weight(in_facility, new.custid, new.item, new.lotnumber,
          new.weight - pl.weight, out_msg);
      if substr(out_msg, 1, 4) != 'OKAY' then
        zms.log_msg('PlateAdj', in_facility, new.custid, out_msg, 'W', in_userid, out_msg);
      end if;
    end if;
    ...
    if l_use_catch_weights = 'Y' then
      ...
      if substr(out_msg, 1, 4) != 'OKAY' then
        zms.log_msg('PlateAdj', in_facility, pl.custid, out_msg, 'W', in_userid, out_msg);
      end if;
      zcwt.add_item_lot_catch_weight(in_facility, pl.custid, pl.item, pl.lotnumber,
          - pl.weight, out_msg);
      if substr(out_msg, 1, 4) != 'OKAY' then
        zms.log_msg('PlateAdj', in_facility, new.custid, out_msg, 'W', in_userid, out_msg);
      end if;
  ...
  --
    ...
    if new.qty > 0 then
      ...
      if odr_cnt = 0 and pl.orderid is not null then
        ...
        values
         (pl.orderid,pl.shipid,new.item,new.lotnumber,in_facility,new.custid,
...
if not ((oh.orderstatus = 'A') or
  ...
  if pl.custid = new.custid and
    ...
    if substr(out_msg,1,4) != 'OKAY' then
        zms.log_msg('PlateAdj', in_facility, new.custid, out_msg,
        'W', in_userid, out_msg);
    end if;
  elsif pl.custid != new.custid or
        pl.item != new.item or
...
if curPlate%notfound then
  close curPlate;
  out_msg := 'Plate record not found';
  out_errorno := 1;
  return;
end if;
close curPlate;

if pl.invstatus = in_newinvstatus then
  out_msg := 'Plate already has this inventory status value';
  out_errorno := 4;
  return;
end if;

if in_newinvstatus = 'QC' then
  if out_controlnumber is null then
    zcn.get_next_controlnumber(out_controlnumber,out_msg);
    if substr(out_msg,1,4) != 'OKAY' then
      out_errorno := 2;
      return;
    end if;
  end if;
  pl.controlnumber := out_controlnumber;
  pl.adjreason := 'QC';
else
  out_controlnumber := null;
  pl.adjreason := in_reason;
end if;
...
if in_newinvstatus = 'QC' then
  update plate
     set controlnumber = pl.controlnumber,
         holdreason = in_reason
   where lpid = in_lpid;
end if;

out_msg := 'OKAY';

end change_invstatus;
...
if pl.inventoryclass = in_newinvclass then
  out_msg := 'Plate already has this inventory class value';
  out_errorno := 4;
  return;
end if;
...
cursor MAI is
  select p.anvdate as oneanvdate, temp.*
  from TEMP_INVENTORY_ADJUSTMENT temp, PLATE p
  where temp.lpid = p.lpid;
...
BEGIN
  ...
  for rec in MAI loop
    inventory_adjustment(rec.lpid,
                         ...
                         rec.orig_custid,
                         rec.orig_item,
                         rec.orig_inventoryclass,
                         rec.orig_invstatus,
                         rec.orig_lotnumber,
                         rec.orig_serialnumber,
                         rec.orig_useritem1,
                         rec.orig_useritem2,
                         rec.orig_useritem3,
                         rec.orig_location,
                         rec.orig_expirationdate,
                         rec.orig_qty,
                         rec.facility,
                         rec.adjreason,
                         rec.userid,
						 ...
						 null,
						 'N',
						 rec.calc_weight_from_item,
						 'MASS');

    if trim(msg) <> 'OKAY' then
      out_errorcount := out_errorcount + 1;
      update TEMP_INVENTORY_ADJUSTMENT
      set    ERRMSG = msg
      where  LPID = rec.lpid;
    end if;
  end loop;

END mass_inventory_adjustment;

/* invoked from db trigger on plate table */
...
l_controlnumber plate.controlnumber%type;
l_errno pls_integer;
l_errmsg appmsgs.msgtext%type;
l_msg appmsgs.msgtext%type;
l_adj1rowid rowid;
l_adj2rowid rowid;
l_rategroup  custrate.rategroup%type;
l_sysdate date;
...
zia.change_invstatus
    (in_lpid, in_invstatus, in_adjreason, in_tasktype,
     in_userid, l_adj1rowid, l_adj2rowid, l_controlnumber,
     l_errno, l_errmsg, 'Y', 'N');

if l_errmsg != 'OKAY' then
    zms.log_autonomous_msg('LOCSTATCHG', in_facility, in_custid,
      'Unable to change invstatus: ' || l_errmsg,
      'E', in_userid, l_msg);
end if;

if l_adj1rowid is not null then
  zim6.check_for_adj_interface(l_adj1rowid, l_errno, l_errmsg);
  if l_errno != 0 then
    zms.log_autonomous_msg('LOCSTATCHG', in_facility, in_custid, l_errmsg,
                           'E', in_userid, l_msg);
  end if;
end if;

if l_adj2rowid is not null then
  zim6.check_for_adj_interface(l_adj2rowid, l_errno, l_errmsg);
  if l_errno != 0 then
    zms.log_autonomous_msg('LOCSTATCHG', in_facility, in_custid, l_errmsg,
                           'E', in_userid, l_msg);
  end if;
...
zbill.rd_item_rategroup(in_custid, in_item, l_rategroup);

zms.log_autonomous_msg('LOCSTATCHG', in_facility, in_custid,
      'Loc Status Change: ' || in_custid || '/' || l_rategroup || '/' || in_event || '/' ||
       in_facility || '/' || l_sysdate || '/' || in_lpid,
      'I', in_userid, l_msg);

l_orderid := 0;
l_shipid := 0;

for invh in (select h.invoice,h.orderid,d.shipid
               from invoicedtl d, invoicehdr h
              where h.facility = in_facility
                and h.custid = in_custid
                and h.invtype = 'M'
                and h.invstatus = '1'
                and d.invoice = h.invoice
              order by invoice desc)
loop
  l_orderid := invh.orderid;
  l_shipid := invh.shipid;
  exit;
end loop;

for crec in zbill.C_RATE_WHEN(in_custid, l_rategroup, in_event, in_facility, l_sysdate)
loop

  if zbill.rd_rate(rategrouptype(crec.custid, crec.rategroup),
                crec.activity,crec.billmethod,
                l_sysdate, RATE) = zbill.BAD then
    zms.log_autonomous_msg('LOCSTATCHG', in_facility, in_custid,
      'Unable to find rate: ' || crec.custid || '/' || crec.rategroup ||
      '/' || crec.activity || '/' || crec.billmethod,
      'E', in_userid, l_msg);
  end if;

  if RATE.billmethod in (zbill.BM_FLAT, zbill.BM_PLT_COUNT) then
    l_qty := 1;
  else
    l_qty := in_qty;
  end if;

  zrf.single_misc_charge(in_facility,l_orderid,l_shipid,'M',RATE.activity,in_custid,in_item,
                         l_qty,in_baseuom,0,0,0,in_userid,
                         'LiP: ' || in_lpid,in_weight,l_errmsg,crec.billmethod);
  if l_errmsg is not null then
    zms.log_autonomous_msg('LOCSTATCHG', in_facility, in_custid,
      'Misc Charge: ' || l_errmsg,
      'E', in_userid, l_msg);
  end if;
...
exception when others then
  zms.log_autonomous_msg('LOCSTATCHG', in_facility, in_custid, 'Exception: ' || sqlerrm,
                           'E', in_userid, l_msg);
end;

end zinvadj;
/
show error package body zinvadj;
exit;
