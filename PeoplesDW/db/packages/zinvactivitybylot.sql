create or replace package asofinvactlotPKG
AS
	procedure asofinvactlotPROC
...
end asofinvactlotpkg;
/

create or replace procedure asofinvactlotbyitemlotPROC
(aoi_cursor IN OUT asofinvactpkg.aoi_type
,in_custid IN varchar2
,in_facility IN varchar2
,in_item IN varchar2
...
cursor curAsOfDtlActivity(in_facility IN varchar2, in_custid IN varchar2, in_item IN varchar2, in_uom IN varchar2, in_invstatus IN varchar2, in_inventoryclass IN varchar2, in_lotnumber IN varchar2) is
...
cursor curShipmentOrders(in_facility IN varchar2, in_custid IN varchar2, in_item IN varchar2, in_lastupdate IN date, in_effdate IN date, in_lotnumber IN varchar2) is
...
from (select nvl(aoi1.currentqty,0) as currentqty,
...
procedure get_asof_detail(in_facility IN varchar2, in_custid IN varchar2, in_item IN varchar2, in_uom IN varchar2, in_invstatus IN varchar2, in_inventoryclass IN varchar2, in_lotnumber IN varchar2)
is
begin

for aod in curAsOfDtlActivity(in_facility, in_custid, in_item, in_uom, in_invstatus, in_inventoryclass, in_lotnumber)
loop
  ...
  if aod.orderid = 0 or aod.shipid = 0 then
    for cso in curShipmentOrders(in_facility,in_custid,in_item,aod.lastupdate,aod.effdate,in_lotnumber)
    loop
...
<< check_receipt >>
  ...
  if aod.orderid = 0 or aod.shipid = 0 then
    for rp in curReceiptPlates(in_facility,in_custid,in_item,in_lotnumber,
...
<< check_return >>
  ...
  if aod.orderid = 0 or aod.shipid = 0 then
    for rt in curReturnPlates(in_facility,in_custid,in_item,in_lotnumber,
      in_uom,in_invstatus,in_inventoryclass, aod.lastupdate, aod.effdate)
    loop
    ...
    loop
      ...
      loop
        ...
        (sessionid,facility,custid,item,lotnumber,uom,invstatus,
         ...
         reference,po,billoflading,
         weight) values
        (numSessionId,cf.facility,cu.custid,cit.item,aob.lotnumber,aob.uom,aob.invstatus,
        ...
        get_asof_detail(cf.facility, cu.custid, cit.item, aob.uom, aob.invstatus, aob.inventoryclass, aob.lotnumber);

        open curAsOfEndSearch(cf.facility, cu.custid, cit.item, aob.uom, aob.invstatus, aob.inventoryclass, aob.lotnumber);
...
end asofinvactlotbyitemlotPROC;
/

CREATE OR REPLACE PACKAGE Body asofinvactlotPKG AS
--
-- $Id$
--
...
as
begin
	ASOFINVACTLOTBYITEMPROC(aoi_cursor, in_custid, in_facility, 'ALL', in_begdate, in_enddate, in_debug_yn);
end asofinvactlotPROC;
end asofinvactlotPKG;
/

create or replace procedure ASOFINVACTLOTBYITEMPROC
(aoi_cursor IN OUT asofinvactpkg.aoi_type
,in_custid IN varchar2
,in_facility IN varchar2
,in_item IN varchar2
...
as
begin
	ASOFINVACTLOTBYITEMLOTPROC(aoi_cursor, in_custid, in_facility, in_item, 'ALL', in_begdate, in_enddate, in_debug_yn);
end ASOFINVACTLOTBYITEMPROC;
/

create or replace procedure ASOFLOTPROC
(aoi_cursor IN OUT asofinvactpkg.aoi_type
,in_lotnumber IN varchar2
,in_begdate IN date)
as
begin
	ASOFINVACTLOTBYITEMLOTPROC(aoi_cursor, 'ALL', 'ALL', 'ALL', in_lotnumber, in_begdate, sysdate, 'N');
end ASOFLOTPROC;
/

create or replace procedure ASOFINVACTLOTPROC
(aoi_cursor IN OUT asofinvactpkg.aoi_type
,in_custid IN varchar2
,in_facility IN varchar2
,in_begdate IN date
...
show errors package asofinvactlotPKG;
show errors procedure asofinvactlotbyitemlotPROC;
show errors procedure asofinvactlotbyitemPROC;
show errors procedure asoflotPROC;
show errors procedure asofinvactlotPROC;
show errors package body asofinvactlotPKG;
exit;
