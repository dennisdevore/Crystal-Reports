...
begin
  cmdSql := 'drop table qcresultdtl_tmp';
...
cmdSql := 'create table qcresultdtl_tmp (orig_rowid varchar2(18), orig_char_value varchar2(4000))';
curSql := dbms_sql.open_cursor;
dbms_sql.parse(curSql, cmdSql, dbms_sql.native);
cntRows := dbms_sql.execute(curSql);
dbms_sql.close_cursor(curSql);
cmdSql := 'select rowid,notes from qcresultdtl' || 
...
loop
  ...
  cmdIns := 'insert into qcresultdtl_tmp values (''' ||
...
cmdSql := 'alter table qcresultdtl drop column notes';
curSql := dbms_sql.open_cursor;
dbms_sql.parse(curSql,cmdSql,dbms_sql.native);
cntRows := dbms_sql.execute(curSql);
cmdSql := 'alter table qcresultdtl add (notes clob)';
curSql := dbms_sql.open_cursor;
dbms_sql.parse(curSql,cmdSql,dbms_sql.native);
cntRows := dbms_sql.execute(curSql);
cmdSql := 'select orig_rowid,orig_char_value from qcresultdtl_tmp';
curSql := dbms_sql.open_cursor;
dbms_sql.parse(curSql,cmdSql,dbms_sql.native);
dbms_sql.define_column(curSql,1,out_rowid,18);
dbms_sql.define_column(curSql,2,out_varchar2,4000);
...
loop
  ...
  dbms_sql.column_value(curSql,1,out_rowid);
  dbms_sql.column_value(curSql,2,out_varchar2);
  cmdUpd := 'update qcresultdtl set notes = ''' || rtrim(out_varchar2) || '''' ||
