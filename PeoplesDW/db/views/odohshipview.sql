create or replace view ohshipview
(
BILLOFLADING,
PRIORITY,
WEIGHTCOMMIT,
...
decode(nvl(orderhdr.hdrpassthrunum01,0),0,
  zim14.freight_cost(orderhdr.orderid,orderhdr.shipid),
  orderhdr.hdrpassthrunum01),
loads.prono,
orderhdr.prono
...
where orderhdr.orderstatus = orderstatus.code (+)
  ...
  and orderhdr.orderstatus in ('9','X');

comment on table ohshipview is '$Id$';

create or replace view odshipview
(
PRIORITY,
ORDERID,
SHIPID,
...
where orderdtl.orderid = orderhdr.orderid(+)
  ...
  and ( orderhdr.orderstatus in ('9','X') or
        orderhdr.edicancelpending = 'Y');

comment on table odshipview is '$Id$';

create or replace view pkshipview
(orderid
,shipid
,reference
,label
...
sp.trackingno,
nvl(md.cost,0)
from multishipdtl md, orderhdr oh, shippingplate sp
...
comment on table pkshipview is '$Id$';
