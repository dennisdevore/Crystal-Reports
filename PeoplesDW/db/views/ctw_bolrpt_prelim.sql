create or replace view CTW_BOLRPT_PRELIM ( OH_ORDERID, 
...
IATATERTIARYCHEMCODE, IATAQUATERNARYCHEMCODE,OH_CONSIGNEE, BASEQTY, BASEWGHT, BASECUBE, OH_SHIPTO)  as
select
OH.orderid,
OH.shipid,
OH.entrydate,
...
OH.CONSIGNEE,
nvl(zlbl.uom_qty_conv(OH.CUSTID,OD.ITEM,nvl(OD.QTY,0),OD.UOM,CI.BASEUOM),0),
nvl(zlbl.uom_qty_conv(OH.CUSTID,OD.ITEM,nvl(OD.wght,0),OD.UOM,CI.BASEUOM),0),
nvl(zlbl.uom_qty_conv(OH.CUSTID,OD.ITEM,nvl(OD.cube,0),OD.UOM,CI.BASEUOM),0),
OH.SHIPTO
from
orderhdr OH,
consignee CN,
...
nvl(OD.lot,'(none)') = nvl(ODC.lotnumber(+),'(none)') and
OH.shiptype = ST.code (+) and
OH.shipterms = STRM.code (+) and
OH.ordertype = 'O' and
--OD.linestatus <> 'X' and
CU.billfreightto = CNPPD.consignee(+);
comment on table CTW_BOLRPT_PRELIM is '$Id$';
exit;
