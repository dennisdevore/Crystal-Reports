create or replace view assignstopview
(
    custid,
    orderid,
    shipid,
...
select
    ...
    O.orderid,
    O.shipid,
    nvl(C.assign_stop_by_passthru_yn,'N'),
    decode( nvl(substr(C.assign_stop_stop_passthru,-2),'??'),
        '??',0,
...
comment on table assignstopview is '$Id';
